1; // 등록 버튼 클릭 시 게시글이 커뮤니티 main의 post_list에 등록
document
  .querySelector("#post_submit_btn")
  .addEventListener("click", function () {
    const postContentTextarea = document.querySelector(
      'textarea[name="post_content"]'
    ); // name이 post_content인 textarea 불러오기
    let postContent = postContentTextarea.value; // value : 사용자가 textarea에 입력한 텍스트 값
    const postTitleTextarea = document.querySelector(
  ...
  commentCount = 0; */
    ...
    newPost
      ...
      .addEventListener("click", function (e) {
        ...
        // 새 창에 전체 내용 텍스트로 표시
        let fullPostWindow = window.open(
          "",
          "FullPostWindow",
